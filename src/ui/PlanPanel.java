...
public class PlanPanel extends JPanel {
    ...
    public PlanPanel(MeasurementApp app) {
        this.app = app;
        setLayout(new BorderLayout(10, 10));
        setBorder(BorderFactory.createEmptyBorder(20, 30, 20, 30));

        JLabel title = new JLabel("Step 3: Plan Measurement", JLabel.CENTER);
        title.setFont(title.getFont().deriveFont(22f));
        add(title, BorderLayout.NORTH);

        tableModel = new DefaultTableModel(new Object[]{"Dimension", "Dim Coeff", "Metric", "Metric Coeff", "Direction", "Range", "Unit"}, 0) {
            @Override
